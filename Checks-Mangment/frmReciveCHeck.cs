...
{
    public partial class frmReciveCHeck : Form
    {
        public frmReciveCHeck()
        {
            InitializeComponent();
        }
        ...
        private void frmReciveCHeck_Load(object sender, EventArgs e)
        {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            {
                OleDbCommand checkCmd = new OleDbCommand("SELECT COUNT(*) FROM ReciveCheck WHERE CheckID = @CheckID", conn);
                ...
                else if (dtReciveDate.Value >= dtDueDate.Value)
                {
                ...
                try
                {
                    OleDbCommand insertcmd = new OleDbCommand("INSERT INTO ReciveCheck VALUES (@CheckID, @CheckNumber, @RecivedDate, @DueDate, @Amount, @PaName,@Bank)", conn);
                    ...
                    insertcmd.Parameters.Add("@RecivedDate", OleDbType.Date).Value = dtReciveDate.Value;
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dtReciveDate.Value >= dtDueDate.Value)
                {
                    MessageBox.Show("You cannot but the Due Date before or Equal the recive date.", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1);
                    return;
                }
                try
                {
                    OleDbCommand updateCmd = new OleDbCommand("UPDATE ReciveCheck SET CheckNumber = @CheckNumber, RecivedDate = @RecivedDate, DueDate = @DueDate, Amount = @Amount, PaName = @PaName, Bank = @Bank WHERE CheckID = @CheckID", conn);

                    updateCmd.Parameters.Add("@CheckNumber", OleDbType.Integer).Value = int.Parse(txtChNum.Text);
                    updateCmd.Parameters.Add("@RecivedDate", OleDbType.Date).Value = dtReciveDate.Value;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (res == DialogResult.Yes)
            {
                    try
                    {
                        OleDbCommand deleteCmd = new OleDbCommand("DELETE FROM ReciveCheck WHERE CheckID = @CheckID", conn);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                OleDbCommand viewallcmd = new OleDbCommand("select * from ReciveCheck", conn);
                OleDbDataAdapter da = new OleDbDataAdapter(viewallcmd);
                DataSet ds = new DataSet();
                da.Fill(ds, "RChk");
                dgv.DataSource = ds.Tables[0];
                DataTable tab = ds.Tables["RChk"];
                if (tab.Rows.Count == 0)
                {
